...
from sketbot.exception import *


class IconRandomizerCog(commands.Cog):
    # saved as key value pairs of guild and [(rolename, roleid),..]
    accepted_roles = {}
    # saved as key value pairs of guild and [(channelname, channelid),..]
    listen_channels = {}
    ...
    async def on_connect(self):
        # TODO maybe connect to database here?
        print("CONNECTED")
        pass
    ...
    async def on_ready(self):
        for guild in self.bot.guilds:
            ...
            # FIXME just for testing. Read that stuff from database later
            self.guild_options[guild] = {
                "crop_picture": True, "safe_picture": True, "random_icon_change": False, "delete_interval": 0}

        print("READY")

        # TODO do that later
        # for guild in self.bot.guilds:
        #     self.guild_options[guild], self.listen_channels[guild], self.accepted_roles[guild] = database_ops.recover_guild(
        #         guild)
        ...
        if len(message.attachments) > 0:
            for item in message.attachments:
                self.save_picture(item.url, self.database,
                                  message.guild, message.author, message.id)
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        self.guild_options[guild] = {
            "crop_picture": True, "safe_picture": False, "random_icon_change": False, "delete_interval": 0}
    ...
    async def on_guild_change(self, before: discord.Guild, after: discord.Guild):
        ...
        if before.name != after.name or before.id != after.id:
            database_ops.update_guild(
                self.database, before.name, before.id, after.name, after.id)
    ...
    async def on_command_error(self, ctx: commands.Context, error):
        ...
        if isinstance(error, database_ops.DatabaseException):
            print(error.reason)
            return
    ...
    async def add_role(self, ctx: commands.Context):
        ...
        database_ops.add_role(self.database, ctx.guild.name,
                              ctx.guild.id, role.name, role.id)
    ...
    async def add_listen_channel(self, ctx: commands.Context):
        ...
        database_ops.add_channel(
            self.database, ctx.guild.name, ctx.guild.id, channelctx.name, channelctx.id)
        self.listen_channels[ctx.guild].append(
            (channelctx.name, channelctx.id))
    ...
    async def change_save_pictures(self, ctx: commands.Context):
        if utilities.wait_for_query(self.bot, ctx, "To enable/disable the saving of the pictures press ✔️ to enable or ❌ to disable it", None, 20):
            database_ops.update_guild_options(
                self.database, ctx.guild.name, ctx.guild.id, safe_picture=True)
            self.guild_options[ctx.guild]["safe_picture"] = True
        else:
            database_ops.update_guild_options(
                self.database, ctx.guild.name, ctx.guild.id, safe_picture=False)
    ...
    async def randomize_server_icon(self, ctx: commands.Context):
        if utilities.wait_for_query(self.bot, ctx, "To enable/disable the randomization of the server icon press ✔️ to enable or ❌ to disable it", None, 20):
            database_ops.update_guild_options(
                self.database, ctx.guild.name, ctx.guild.id, random_icon_change=True)
            self.guild_options[ctx.guild]["random_icon_change"] = True
        else:
            database_ops.update_guild_options(
                self.database, ctx.guild.name, ctx.guild.id, random_icon_change=False)
    ...
    async def crop_picture(self, ctx: commands.Context):
        if utilities.wait_for_query(self.bot, ctx, "If you want that every picture will get compressed to the recommended size of a discord server icon press ✔️ to enable or ❌ to disable it", None, 20):
            database_ops.update_guild_options(
                self.database, ctx.guild.name, ctx.guild.id, crop_picture=True)
            self.guild_options[ctx.guild]["crop_picture"] = True
        else:
            database_ops.update_guild_options(
                self.database, ctx.guild.name, ctx.guild.id, crop_picture=False)
    ...
    def save_picture(self, url: str, database, guild: discord.Guild, author: discord.User, messageid: int):
        ...
        if(database.is_connected()):
            if self.guild_options[guild]["safe_picture"]:
                imagepath = f"{self.image_folderpath}/{guild.id}/{messageid}.png"
                try:
                    if database_ops.add_picture(
                            self.database, hash, guild.name, guild.id, author.name, author.id, img.width, img.height, imagepath):
    ...
    def delete_guild_folder(self, guild: discord.Guild):
        shutil.rmtree(
            f'{self.image_folderpath}/{str(guild.id)}', ignore_errors=True)

    def rename_guild_folder(self, before: discord.Guild, after: discord.Guild):
        os.rename(f'{self.image_folderpath}/{str(before.id)}',
                  f'{self.image_folderpath}/{str(after.id)}')
